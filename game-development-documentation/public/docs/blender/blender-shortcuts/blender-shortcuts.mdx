# Blender Shortcuts

## General Navigation & Interface

| Shortcut | Description | Image |
|----------|-------------|-------|
| **Middle Mouse Button** or **Two-finger trackpad**| Rotate view |
| **Shift + Middle Mouse** or **Shift + Two-finger trackpad**| Pan view |
| **Scroll Wheel** or **Ctrl + Ctrl-Key + Two-finger trackpad** | Zoom in/out |
| **Shift + ~** | Fly mode | ![](./assets/fly-mode.gif) |
| **Q** | Quick Favorites | ![](./assets/quick-favorites.gif) |
| **/** | Toggle Local View | ![](./assets/toggle-local-view.gif) |
| **Shift + R** | Repeat last action | ![](./assets/repeat-last-action-shift-r.gif) |
| **F3** | Command Pallete/Search Menu | ![](./assets/command-panel-or-search-menu.gif) |
| **Ctrl + Space** | Fullscreen window | ![](./assets/fullscreen-window.gif) |

## Selection & Object Management

| Shortcut | Description |
|----------|------------|
| **A** | Select all |
| **Alt + A** | Deselect all |
| **Ctrl + I** | Invert selection |
| **B** | Box select |
| **C** | Circle select |
| **L** | Select linked |
| **Shift + L** | Select linked (menu) |
| **Ctrl + L** | Make links menu |

## Transform Operations

| Shortcut | Description | Image |
|----------|------------|--------|
| **G** | Grab/move |
| **R** | Rotate |
| **S** | Scale |
| **Alt + G** | Remove Move | ![](./assets/alt-g-remove-move.gif) |
| **Alt + R** | Remove Rotate | ![](./assets/alt-r-remove-rotation.gif) |
| **Alt + S** | Remove Scale | |
| **Alt + S** While normals are selected | Scale along the normals | ![](./assets/scale-along-normals.gif)
| **Shift + S** | Snap menu |
| **Shift + Right Click** | Move 3D Cursor to cursor |
| **Tab** | Toggle Edit/Object mode |
| **Shift + A** | Add menu |
| **X** or **Delete** | Delete menu |
| **Ctrl + A** | Apply transformations menu |
| **Ctrl + J** | Join selected objects |
| **P** | Separate (in Edit mode) |
| **Alt + D** | Duplicate linked |
| **Shift + D** | Duplicate |

## Axis Constraints (During Transform)

| Shortcut | Description | Images | 
|----------|------------|---------|
| **X** | Constrain to X-axis |
| **Y** | Constrain to Y-axis |
| **Z** | Constrain to Z-axis |
| **X, Y, Z** (double) | Uses local orientation | ![](./assets/transform-local-orientation.gif) |
| **Shift + X** | Constrain to YZ plane |
| **Shift + Y** | Constrain to XZ plane |
| **Shift + Z** | Constrain to XY plane |

## Transform Modifiers

| Shortcut | Description |
|----------|------------|
| **Shift** (during transform) | Precise movement |
| **Ctrl** (during transform) | Snap to grid |
| **Alt** (during transform) | Move in small increments |


## Workspace & UI

| Shortcut | Description | Image |
|----------|-------------|-------|
| **T** | Toggle toolbar | ![](./assets/toggle-toolbar.gif) |
| **N** | Toggle properties panel | ![](./assets/properties-panel.gif) |
| **Ctrl + Spacebar** | Fullscreen window | ![](./assets/ctrl-spacebar-fullscreen-window.gif)